...
void menuCanvas::mouseDragged(int x, int y, int button) {
	for(int i = 0; i < OPTIONS_COUNT; i++) {
		if(sliderselected[i] && i == SLIDER_MUSIC) {
			if(slider[i].x >= sliderminx[i] && slider[i].x <= slidermaxx[i]) slider[0].x = x;
			if(slider[i].x < sliderminx[i]) slider[i].x = sliderminx[i];
			if(slider[i].x > slidermaxx[i]) slider[i].x = slidermaxx[i];
//			gLogi("Value ") << std::to_string(normalizeSlider(sliderminx[i], slidermaxx[i], slider[i].x));
		}
	}
}
...
void menuCanvas::drawOptionsMenu() {
	...
	opbuttons[ACCEPT_BUTTON].drawSub(opbutton[ACCEPT_BUTTON].x, opbutton[ACCEPT_BUTTON].y, opbutton[ACCEPT_BUTTON].w, opbutton[ACCEPT_BUTTON].w, 0, 0, opbutton[ACCEPT_BUTTON].sw, opbutton[ACCEPT_BUTTON].sh);
}
...
void menuCanvas::soundControl(bool musicstate) {
	if(!musicstate) {
		root->music.stop();
